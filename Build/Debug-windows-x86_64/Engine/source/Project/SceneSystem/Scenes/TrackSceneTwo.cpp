...
namespace Project
{
	...
	void TrackSceneTwo::RenderScene()
	{
		GUI();
	...
	bool TrackSceneTwo::VehicleOverFinishLine()
	{
		if (m_PhysicsEntityManager->GetEntity("MainCar"))
		{
			...
			if (entity->GetComponent("Transform"))
			{
				...
				if (comp->GetPosition().z > 30.0f && comp->GetPosition().z < 30.5f &&
					comp->GetPosition().x > -15.0f && comp->GetPosition().x < 15.0f)
				{
					return true;
				}
				else
