...
namespace Project
{
	class OpenWorldSceneTwo : public IScene
	{
	public:
		OpenWorldSceneTwo(CDirectX11SceneManager* sceneManager, std::shared_ptr<IRenderer>, int sceneIndex, CVector3 ambientColour = CVector3(1.0f, 1.0f, 1.0f),
			float specularPower = 256.0f, ColourRGBA backgroundColour = ColourRGBA(0.2f, 0.2f, 0.3f, 1.0f),
			bool vsyncOn = true);

		OpenWorldSceneTwo(CDirectX11SceneManager* sceneManager, std::shared_ptr<IRenderer>, bool enablePhysics, int sceneIndex, CVector3 ambientColour = CVector3(1.0f, 1.0f, 1.0f),
			float specularPower = 256.0f, ColourRGBA backgroundColour = ColourRGBA(0.2f, 0.2f, 0.3f, 1.0f),
			bool vsyncOn = true);
